                        ...
                        <div className="flex flex-col gap-2">
                            <div className="bg-base-100 rounded shadow p-3 flex flex-col gap-5">
                                ...
                                <div className="flex flex-col gap-3 w-full overflow-hidden text-center">
                                    <img src={Req.Manhwa.concat(Req.Hentai).find(doc => doc.Slug == Req.Cookies.slice(-1))?.Image} className="flex-none rounded-full w-20 h-20 m-auto" width="240" height="320" lazy='loading' alt={Req.Manhwa.concat(Req.Hentai).find(doc => doc.Slug == Req.Cookies.slice(-1))?.Title} />
                                    
                                    <h1 className="text-sm font-bold">
                                        {Req.Manhwa.concat(Req.Hentai).find(doc => doc.Slug == Req.Cookies.slice(-1))?.Title}
                                    </h1>
                                    
                                    <div className="flex flex-row gap-2 w-full">
                                        <a href='/bookmark' className='btn btn-sm btn-primary normal-case' aria-label='UnBookmark' onClick={(event) => {
                                                const RemoveBookmark = Req.Cookies.filter((dos) => {return dos != Req.Hentai.find(doc => doc.Slug == Req.Cookies.slice(-1))?.Slug});
                                        ...
                                        <div className="w-full">
                                            <a href={`${Req.Manhwa.concat(Req.Hentai).find(doc => doc.Slug == Req.Cookies.slice(-1))?.Type}/${Req.Manhwa.concat(Req.Hentai).find(doc => doc.Slug == Req.Cookies.slice(-1))?.Slug}`} className="btn btn-sm btn-block btn-primary"> More Information </a>
                                        </div>              
                                    </div>
                                </div>
                            </div>
